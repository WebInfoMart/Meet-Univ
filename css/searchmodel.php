...
$query=$this->db->get();
return $query->result_array();
}function fetch_program_id(){	$this->db->select('prog_parent_id');	$this->db->from('program_parent');	$this->db->where('program_parent_name',$this->input->post('program'));			$query=$this->db->get();	$result=$query->result_array();	$id=$result[0]['prog_parent_id'];	$this->db->select('prog_id,course_name');	$this->db->from('program');	$this->db->where('prog_parent_id',$id);	if($this->input->post('level'))	{	$this->db->where_in('educ_level_id',$this->input->post('level'));	}	else	{				$level=array(2,3,4);		$this->db->where_in('educ_level_id',$level);	}//$this->db->limit(20);$query=$this->db->get();return $query->result_array();}
...
}function get_educ_sublevel_id_by_name($educ_sublevel,$parent_id,$edu_level){	$this->db->select('prog_id');	$this->db->from('program');	$this->db->where('educ_level_id',$edu_level);	$this->db->where('prog_parent_id',$parent_id);	$this->db->where('course_name',$educ_sublevel);	$res=$this->db->get();	if($res->num_rows()>0)	{	return $res->row_array();	}	else	{	return 0;	}}
...
{//echo $current_url;exit;
		...
		$univ_data['filter_educ_level']=array(0);		$univ_data['filter_educ_sublevel']=array(0);
		//$current_url=str_replace('-',',',$current_url);
		$current_url=str_replace('_',' ',$current_url);
		//$univ_data['title']='Colleges | MeetUniversities';
		$pos = strpos($current_url,'colleges/');
		$filter_country=0;
		$filter_educ_level=0;		$filter_educ_sublevel=0;
		$filter_area_interest=0;
		 $title1='';
		 $title2='';
		 $title3='';		 $title4='';		 		 
		...
		for($f=0;$f<count($filter_content);$f++)
		{
		  ...
		  if($chk_country!=0)
		  {
		   if($title1=='')
		   {
		   $title1=' in '.ucwords($filter_content[$f]);
		   }
		   else
		   {
		  ...
		  if($chk_educ_level!=0)
		  {
			  if($title2=='')
			  {
			  $title2=ucwords($filter_content[$f]);
			  }
			  else
			  {
			  $title2.=','.ucwords($filter_content[$f]);
		  ...
		  if($chk_area_intrest!=0)
		  {
			  ...
			  if($title3=='')
			  {
			  $title3.=ucwords($filter_content[$f]);
			  }
			  else
			  {
			  $title3.=' , '.ucwords($filter_content[$f]);
			  }
			  //$title3='  in '.$filter_content[$f];			  $univ_data['filter_area_intrest'][]=$chk_area_intrest['prog_parent_id'];				continue;
		  }		  			
		  			//kk			if(!empty($area_interest) && !empty($educ_level) )			{				 $filter_content[$f]=str_replace('-','(',$filter_content[$f]);				 $filter_content[$f]=str_replace('~',')',$filter_content[$f]);				 $subcourse=$this->searchmodel->get_educ_sublevel_id_by_name($filter_content[$f],$area_interest[0],$educ_level[0]);							  if($subcourse!=0)				  {					  if($title4=='')					  {					  $title4=ucwords($filter_content[$f]);					  }					  else					  {					  $title4.=','.ucwords($filter_content[$f]);					  }					  $educ_sublevel[]=$subcourse['prog_id'];					  $filter_educ_sublevel=1;				  }				   $univ_data['filter_educ_sublevel'][]=$subcourse['prog_id']; 					continue;				  //kk			}
		  }
		}
		}
		}
		...
		{
		$title4.=' in ';
		}				if($title1!='' || $title2!='' || $title3!='' || $title4!='')		{		$title.='Colleges '.$title1.' '.$title2.' '.$title4.' '.$title3.' | Meet Universities';		}
		else
		{
		$title='Colleges | Meet Universities';
		}
		$univ_data['total_res']=0;
		$univ_data['limit_res']=25;
		...
		if($filter_area_interest==1)
		{
			...
			$area_interest_ids=implode(',',$area_interest);
			$where.=" and univ_program.prog_parent_id IN(".$area_interest_ids.")";
		}		if($filter_educ_sublevel==1)		{			if(!($filter_educ_sublevel==1 &&  $filter_area_interest==1))			$join.=' JOIN univ_program ON univ_program.univ_id=university.univ_id';			$educ_sublevel_ids=implode(',',$educ_sublevel);			$where.=" and univ_program.program_id IN(".$educ_sublevel_ids.")";		}
